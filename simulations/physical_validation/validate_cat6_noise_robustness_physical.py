...
for N in N_vals:
    for sigma in sigmas:
        ...
        for si in range(n_seeds):
            ...
            for p in range(P):
                # Shared deterministic topology: qa.reset_epoch refreshes the
                # world with the canonical epoch seed; nb3.reset_epoch() leaves
                # the world untouched, so both agents see the same pathset.
...
result = {
    "pass": pass_cat6,
    "reason": (
        f"QA-MAB win rate at N=20: "
        f"sigma=0={table[20][0.0]:.1%}, "
        f"sigma=0.05={table[20][0.05]:.1%} — "
        f"{'PASS' if pass_cat6 else 'FAIL'}"
    ),
...
ax.set_xlabel("N flows"); ax.set_ylabel("sigma")
ax.set_title("QA-MAB win rate (vs NB3R)")
