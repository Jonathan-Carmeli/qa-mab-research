"""quick_final.py — fast SA-medium test, N=10 only, T=500"""
...
def run_exp(N, T, n_seeds, learn_fn, sa_nr, sa_ni, label):
    sw_ratios = []
    for si in range(n_seeds):
        ...
        for t in range(T):
            ...
            assign = sa(q, u_hat, N, 4, rng, sa_nr, sa_ni)
...
print('SA-MEDIUM TEST — tau_cap=5.0, N=10, T=500, 20 seeds')
print('='*55)
results = {}
for name, lfn in [('noI_SAmed',l_noI),('Aonly_SAmed',l_Aonly),('baseline_SAmed',l_baseline)]:
    mr, sr = run_exp(10, T, N_SEEDS, lfn, 50, 200, name)
    results[name] = {'sw_ratio':mr,'sw_std':sr}
print()
print('SA-WEAK TEST (for comparison)')
print('='*55)
for name, lfn in [('noI_SAweak',l_noI),('Aonly_SAweak',l_Aonly),('baseline_SAweak',l_baseline)]:
    mr, sr = run_exp(10, T, N_SEEDS, lfn, 8, 15, name)
    results[name] = {'sw_ratio':mr,'sw_std':sr}
with open(os.path.join(RESULTS,'final_comparison.json'),'w') as f:
