"""ucb_minimal.py — fastest possible, T=300, N=10, 10 seeds"""
import os, sys, json
import numpy as np
...
print('='*55)
configs = [
    ('baseline_SAweak',    sacfg_w, 'baseline'),
    ('ucb_SAweak',       sacfg_w, 'ucb'),
    ('ucb_SAmed',        sacfg_m, 'ucb'),
    ('ucb_noI_SAweak',   sacfg_w, 'ucb_noI'),
    ('ucb_noI_SAmed',    sacfg_m, 'ucb_noI'),
    ('softmax_SAweak',  sacfg_w, 'softmax'),
    ('random',          sacfg_w, 'random'),
]
for name, sacfg, method in configs:
    mr, sr = run(N, T, n, method, sacfg)
    results[name] = {'sw_ratio':mr, 'sw_std':sr}
    print(f'{name}: SW={mr:.4f}±{sr:.4f}')
with open(os.path.join(RESULTS,'ucb_minimal.json'),'w') as f:
