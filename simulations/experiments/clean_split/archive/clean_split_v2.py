"""
clean_split_v2.py — Joint Learning with Error Feedback Correction
"""
...
B_LR = 0.2; I_LR = 0.05; I_CAP = 0.3
UCB_C = 0.5; COLLISION_THRESH = 0.02; T_START = 2.0
...
class CleanSplitV2:
    ...
    def step(self):
        ...
        if self.use_oracle_I:
            ...
            gap = max(0.0, opt_sw - actual_sw)
            avg_I = gap / self.N
            for i in range(self.N):
                k = assignment[i]
                B_obs = tp_actual[i] + avg_I
                self.B_hat[i, k] += B_LR * (B_obs - self.B_hat[i, k])
                self.visits[i, k] += 1
...
def run_exp(use_oracle_I, n_seeds, T_steps):
    ...
    for si in range(n_seeds):
        ...
        algo = CleanSplitV2(env, use_oracle_I=use_oracle_I, seed=si * 1000 + N)
...
def main():
    print("=" * 65)
    print("CLEAN SPLIT QUBO V2 — Joint Error Correction")
    print("=" * 65)
    all_res = {}
    ...
    print("\nPHASE B: Learned I (avg gap correction)")
    ...
    out = os.path.join(RESULTS, 'clean_split_v2_results.json')
