"""ucb_tau_sweep.py — UCB + sweep tau_cap, N=10, T=500, 15 seeds"""
import os, sys, json
import numpy as np
...
def run(N, T, n_seeds, tau_cap, use_ucb, sacfg):
    sw_ratios = []
    for si in range(n_seeds):
        ...
        for t in range(T):
            if use_ucb:
                ...
                assign = sa(bq(u_hat, I_hat, N, 4, tau, 2.0), u_hat, N, 4, rng, **sacfg)
...
print('='*60)
print('tau_cap sweep WITH and WITHOUT UCB')
print('='*60)
for tau_cap in [0.5, 1.0, 2.0, 3.0, 5.0, 7.0, 10.0]:
    mr_baseline, sr_baseline = run(N, T, n, tau_cap, False, sacfg)
    mr_ucb, sr_ucb = run(N, T, n, tau_cap, True, sacfg)
    results[f'tau{tau_cap}_baseline'] = {'sw_ratio':mr_baseline, 'sw_std':sr_baseline}
    results[f'tau{tau_cap}_ucb'] = {'sw_ratio':mr_ucb, 'sw_std':sr_ucb}
    print(f'tau_cap={tau_cap}: baseline={mr_baseline:.4f}  ucb={mr_ucb:.4f}  delta={mr_ucb-mr_baseline:+.4f}')
with open(os.path.join(RESULTS,'ucb_tau_sweep.json'),'w') as f:
