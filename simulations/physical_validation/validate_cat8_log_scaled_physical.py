#!/usr/bin/env python3
"""Cat 8: QA-MAB (temperature-scaled QUBO) vs Log-Scaled QUBO.
...
Ported from simulations/legacy/validate_cat8_log_scaled.py — imports
adjusted to the physical_validation/ layout (simulations.physical_validation.*).
"""
import sys, os
...
def main(N_vals=None, P=5, T=100, n_seeds=10):
    ...
    print(f"  config: N_vals={N_vals}, P={P}, T={T}, n_seeds={n_seeds}", flush=True)
    rows = []
    qa_wins = 0
    log_wins = 0
    ...
    for N in N_vals:
        ...
        for si in range(n_seeds):
            ...
            for p in range(P):
                qa.reset_epoch(p)
                log_qa.reset_epoch(p)
                for t in range(T):
                    c_qa = qa.act(t, p)
                    l_qa = world.compute_losses(c_qa, rng)
                    qa.update(c_qa, l_qa)
                    qa_losses.append(l_qa.mean())

                    c_log = log_qa.act(t, p)
                    l_log = world.compute_losses(c_log, rng2)
                    log_qa.update(c_log, l_log)
                    log_losses.append(l_log.mean())

            qa_finals.append(np.mean(qa_losses[-10:]))
            log_finals.append(np.mean(log_losses[-10:]))

        qa_arr = np.array(qa_finals)
        log_arr = np.array(log_finals)
        qa_mean = float(qa_arr.mean())
        ...
        rows.append({
            ...
            "log_mean": log_mean,
            "qa_better_rate": wr,
            "diff": qa_mean - log_mean,
        })
        print(f"    N={N}: gamma_mean={qa_mean:.3f}  log_mean={log_mean:.3f}  "
              f"diff={qa_mean - log_mean:+.3f}  qa_better_rate={wr:.2f}", flush=True)
    ...
    print("  Per-step regret tracking (N=10, 3 seeds)...", flush=True)
    ep_qa_all = []
    ep_log_all = []
    for si in range(3):
        seed = 42 + si
        ...
        for p in range(P):
            ...
            for t in range(T):
                ...
                c = log_qa.act(t, p)
                l = world.compute_losses(c, rng2)
                log_qa.update(c, l)
                ep_log.append(l.mean())
        ep_qa_all.append(ep_qa)
        ep_log_all.append(ep_log)

    avg_qa = np.mean(ep_qa_all, axis=0)
    avg_log = np.mean(ep_log_all, axis=0)
    windows = 10
    rw_qa = [float(np.mean(avg_qa[i * windows:(i + 1) * windows]))
             for i in range(len(avg_qa) // windows)]
    rw_log = [float(np.mean(avg_log[i * windows:(i + 1) * windows]))
              for i in range(len(avg_log) // windows)]

    result = {
        "pass": True,
        "reason": f"QA-MAB (gamma) wins {qa_wins}/{len(rows)} N values, Log wins {log_wins}/{len(rows)}",
        "config": {"N_vals": N_vals, "P": P, "T": T, "n_seeds": n_seeds, "K": 4, "m": 20, "Z": 6},
        "rows": rows,
        "per_window_qa": rw_qa,
    ...
    plt.xlabel("Step")
    plt.ylabel("Mean Loss (last 10 steps)")
    ...
    for r in rows:
        print(f"  N={r['N']}: gamma={r['qa_mean']:.3f}  log={r['log_mean']:.3f}  "
              f"diff={r['diff']:+.3f}  qa_better_rate={r['qa_better_rate']:.2f}")
