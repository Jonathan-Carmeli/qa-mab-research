"""epsilon_ucb_test.py — N=10, T=500, 20 seeds
KEY FIX: tau_cap=5 makes one-hot penalty = 2.5 >> u_hat differences
Weaken penalty OR add exploration.
"""
import os, sys, json
import numpy as np
        ...
        for t in range(T):
            if method == 'baseline':
                ...
                if rng.random() < 0.2:
                    ...
                    assign = sa(bq(u_hat, I_hat, N, 4, tau), u_hat, N, 4, rng, 8, 15)
            elif method == 'weak_penalty':
                # lambda=0.5 instead of 2.0
                q = np.zeros((N*4, N*4))
                for i in range(N):
                    for k in range(4): q[i*4+k,i*4+k] = -u_hat[i,k]
                for i in range(N):
                    for k in range(4):
                        for l in range(4):
                            if k!=l: q[i*4+k,i*4+l] += tau*0.5/2  # weak
                assign = sa(q, u_hat, N, 4, rng, 8, 15)
            elif method == 'ucb':
                # UCB bonus in diagonal
                q = np.zeros((N*4, N*4))
                for i in range(N):
                    for k in range(4):
                    ...
                    for i in range(N):
                        for k in range(4):
                            for l in range(4):
                                if k!=l: q[i*4+k,i*4+l] += tau
                    assign = sa(q, u_hat, N, 4, rng, 8, 15)
            elif method == 'greedy_first':
                # First 100 steps: pure random to explore
                if t < 100:
                    assign = {i: rng.integers(0,4) for i in range(N)}
                else:
                    assign = sa(bq(u_hat, I_hat, N, 4, tau), u_hat, N, 4, rng, 8, 15)
            elif method == 'greedy_first_samed':
                if t < 100:
                    assign = {i: rng.integers(0,4) for i in range(N)}
                else:
                    assign = sa(bq(u_hat, I_hat, N, 4, tau), u_hat, N, 4, rng, 50, 200)
            else:
                assign = sa(bq(u_hat, I_hat, N, 4, tau), u_hat, N, 4, rng, 8, 15)
...
configs = [
    ...
    ('eps_greedy',          'ε-greedy (ε=0.2)'),
    ('weak_penalty',        'Weak penalty (λ=0.5)'),
    ('ucb',                 'UCB bonus only'),
    ('ucb_eps',             'UCB + ε-greedy'),
    ('greedy_first',        'Random@100 then SA-weak'),
    ('greedy_first_samed',  'Random@100 then SA-medium'),
]
for name, desc in configs:
    mr, sr = run(N, T, n, name, desc)
    results[name] = {'sw_ratio':mr, 'sw_std':sr}
    print(f'{desc}: SW={mr:.4f}±{sr:.4f}')
with open(os.path.join(RESULTS,'epsilon_ucb.json'),'w') as f:
